...
void string_add_bool(int val)
{
	if (val){string_add_int(1);}else {string_add_int(0);}
}
void string_add_plusminus(int val)
{
	if (val){string_add_char('+');}else {string_add_char('-');}
}
...
void string_add_space_to_end(int str_length){
	while (strlen(string_buffer) < str_length)
	{
		string_add_space();
	}
}